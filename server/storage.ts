...
export interface IStorage {
  ...
  updateSharedTableRemark(shareId: string, remark: string): Promise<SharedTableState | undefined>;
...
export class MemStorage implements IStorage {
  ...
  async bulkUpdateMarkerColorByRoute(route: string, color: string): Promise<number> {
    let updatedCount = 0;
  ...
  async updateSharedTableRemark(shareId: string, remark: string): Promise<SharedTableState | undefined> {
    const existing = this.sharedStates.get(shareId);
    if (!existing) return undefined;
    const updated: SharedTableState = { ...existing, remark };
...
export class DatabaseStorage implements IStorage {
  ...
  async deleteCustomTableRows(customTableId: string): Promise<boolean> {
    const result = await db
      .delete(customTableRows)
      .where(eq(customTableRows.customTableId, customTableId));
    return result.rowCount ? result.rowCount > 0 : false;
  }

  async createCustomTableRow(row: InsertCustomTableRow): Promise<CustomTableRow> {
    const [created] = await db
      .insert(customTableRows)
      .values(row)
      .returning();
